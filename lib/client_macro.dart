...
macro class API implements ClassTypesMacro, ClassDeclarationsMacro {
  ...
  // @override
  // Future<void> buildTypesForLibrary(Library library, TypeBuilder builder) {
  //
  //
  //   for (final clazz in classes) {
  //     builder.declareType(clazz[0], DeclarationCode.fromString(clazz[1]));
  //   }
  //
  ...
  Future<void> buildTypesForClass(ClassDeclaration clazz,
    ...
    final route = await builder.resolveIdentifier(
        Uri.parse("package:client_macro/client_macro.dart"), "Route");

    final routeType = NamedTypeAnnotationCode(name: route);

    root.buildClass(builder, routeType);
...
//     builder.declareInLibrary(DeclarationCode.fromString(r"""
...
String _withIntend(String? content) {
  if (content == null) {
    return "";
  }
  return content.split("\n").map((e) => "$_intend$e").join("\n");
}
...
class RouteItem {
  ...
  void buildClass(TypeBuilder builder, NamedTypeAnnotationCode parentType) {
...
${child.privateClassName} get ${child.camelCase} => ${child
    ...
    builder.declareType(
        privateClassName, DeclarationCode.fromParts([
      "class $privateClassName extends ",
      parentType,
      " {",
      """
${_withIntend(currentParam)}
${_withIntend(otherParams)}
${_withIntend(constructor)}
${_withIntend("@override")}
${_withIntend("String get route => \"$builtRoute\";")}

${_withIntend(getters.join("\n"))}

${_withIntend(callMethod)}
    ...
    for (final child in children) {
      child.buildClass(builder, parentType);
    }
  }
